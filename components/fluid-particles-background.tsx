...
export function FluidParticlesBackground({
  ...
  particleSize = { min: 0.45, max: 1.55 },
  className,
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (const particle of particles) {
        ...
        const opacity = Math.sin((particle.life / particle.maxLife) * Math.PI) * 0.13;
        const n = noise.simplex3(particle.x * noiseIntensity, particle.y * noiseIntensity, time);
        const angle = n * Math.PI * 4;
        const speed = prefersReduced ? 0.45 : 1.45;
